...
export default function Edit({ item }) {
    const { data, setData, put, processing, errors } = useForm({
        date: item.data.date || '',
        prix: item.data.prix || '',
        station_id: item.data.station_id || 7,
        n_bon: item.data.n_bon || '',
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('factures.update', item.data.id));
    };

    return (
        <DashboardLayout>
            <Head title="Edit facture" />
                        ...
                        <div className="p-6">
                            ...
                            <form onSubmit={handleSubmit} className="space-y-6">
                                ...
                                <div className="flex items-center justify-end space-x-3 border-t dark:border-gray-700 pt-6">
                                    ...
                                    >
                                        {processing ? 'Saving...' : 'Update Facture'}
                                    </button>
                                </div>
                            </form>
                        </div>
